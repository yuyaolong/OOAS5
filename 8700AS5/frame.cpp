...
void Frame::draw(Sint16 x, Sint16 y, double angle, double zoomx, double zoomy, double scale) const {
    //SDL_Surface* tmp0 = rotozoomSurface(surface, 0, scale, SMOOTHING_ON);
    SDL_Surface* tmp = rotozoomSurfaceXY(surface, angle, zoomx*scale, zoomy*scale, 1);
    Sint16 zero = 0;
    Uint16 width = tmp->w;
    Uint16 height = tmp->h;
    SDL_Rect src = { zero, zero, width, height };
